import win32gui
import torch
import cv2
import numpy as np
from PIL import ImageGrab
from utils.augmentations import letterbox
from utils.general import (non_max_suppression, scale_coords)
from models.experimental import attempt_load

# 置信率
conf_thres = 0.2
# 默认0.45效果较好
iou_thres = 0.5
# RGB  用来画框的颜色
color = (0, 255, 0)
# 权重文件名
weights = 'best.pt'
# 目标窗口类名
wnd_name = 'QQMusic_Daemon_Wnd'
# 找到目标窗口
hwnd = win32gui.FindWindow(wnd_name, None)
# 获取目标窗口位置-屏幕上
x1, y1, x2, y2 = win32gui.GetWindowRect(hwnd)
#x1, y1, x2, y2 =11, 224, 1281, 973
width = x2 - x1
height = y2 - y1
# 识别矩形区域位置
rect = (x1, y1, x2, y2)

def run():
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(device)
    # 判断设备类型并仅使用一张GPU进行测试
    half = device != 'cpu'
    # 载入模型
    model = attempt_load(weights, device=device)
    # 将模型的stride赋给stride变量 32
    stride = max(int(model.stride.max()), 32)  # model stride
    model.half()  # to FP16
    while True:
        # 记录识别对象坐标
        tars = []
        # 截取目标区域图像
        im = ImageGrab.grab(bbox=rect)
        # 将图像数据转换成np数组
        img0 = np.array(im)
        # 将图像缩放到指定尺寸
        img = letterbox(img0, stride=stride)[0]
        # 函数将一个内存不连续存储的数组转换为内存连续存储的数组,使得运行速度更快
        img = np.ascontiguousarray(img)
        # 把数组转换成张量,且二者共享内存
        img = torch.from_numpy(img).to(device)
        img = img.half() if half else im.float()
        # 压缩数据维度
        img /= 255  # 0 - 255 to 0.0 - 1.0
        if len(img.shape) == 3:
            img = img[None]
        # 对tensor进行转置
        img = img.permute(0, 3, 1, 2)
        # Inference 模型推理
        pred = model(img, augment=False, visualize=False)[0]
        # NMS 非极大值抑制 即只输出概率最大的分类结果
        pred = non_max_suppression(pred, conf_thres, iou_thres)
        # 处理预测识别结果
        for i, det in enumerate(pred):  # per image
            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = scale_coords(img.shape[2:], det[:, :4], img0.shape).round()
                # 框选出检测结果
                for *xyxy, conf, cls in reversed(det):
                    cv2.rectangle(img0, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), color, 3)
                    # 添加识别对象坐标
                    tars.append((int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])))
        # 调用CV显示结果图
        b, g, r = cv2.split(img0)
        image_1 = cv2.merge([r, g, b])
        resized_img = cv2.resize(np.array(image_1), (width, height))
        # 设置窗口可调整大小
        cv2.namedWindow("display", cv2.WINDOW_NORMAL)
        cv2.imshow("display", resized_img)
        cv2.waitKey(1)
if __name__ == "__main__":
    run()




'''
        # 图像捕捉
        im = ImageGrab.grab(bbox=rect)
        img0 = np.array(im)
        # 图像预处理
        img = letterbox(img0, stride=stride)[0]
        img = np.ascontiguousarray(img)
        img = torch.from_numpy(img).to(device)
        img = img.half() if half else img.float()
        img /= 255.0
        if len(img.shape) == 3:
            img = img[None]
        img = img.permute(0, 3, 1, 2)  # 转置以匹配模型输入
        # 模型推理
        pred = model(img, augment=False, visualize=False)[0]
        # 非极大值抑制
        pred = non_max_suppression(pred, conf_thres, iou_thres)
        # 结果显示
        for i, det in enumerate(pred):
            if len(det):
                det[:, :4] = scale_coords(img.shape[2:], det[:, :4], img0.shape).round()
                for *xyxy, conf, cls in reversed(det):
                    cv2.rectangle(img0, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), color, 3)
'''












